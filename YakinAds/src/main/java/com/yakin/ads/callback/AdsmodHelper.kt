...
object AdsmodHelper {
    ...
    fun increaseNumClickAdsPerDay(context: Context, idAds: String?) {
        val pre = context.getSharedPreferences(
            FILE_SETTING_ADMOD,
            Context.MODE_PRIVATE
        )
    ...
    fun setupAdmodData(context: Context) {
        ...
        val firstTime = context.getSharedPreferences(
            FILE_SETTING_ADMOD,
            Context.MODE_PRIVATE
        ).getLong(KEY_FIRST_TIME, System.currentTimeMillis())
    ...
    private fun resetAdmodData(context: Context) {
        context.getSharedPreferences(FILE_SETTING_ADMOD, Context.MODE_PRIVATE)
            .edit()
            .clear()
            .apply()
        context.getSharedPreferences(
    ...
    private fun isFirstOpenApp(context: Context): Boolean {
        return context.getSharedPreferences(
            FILE_SETTING,
            Context.MODE_PRIVATE
        ).getBoolean(IS_FIRST_OPEN, false)
    }
}